...
type VoucherBatch = { id: number; name: string; status: string; quantity: number; expiresAt: string; instituteName: string; courseTitle: string | null; available: number; redeemed: number };
type Coupon = { id: number; name: string; codeHint: string; status: string; courseTitle: string | null; discountType: string; discountValue: string; expiresAt: string; redemptionCount: number; maxRedemptions: number | null };
...
export function AdminCertificationBenefits() {
  ...
  const [voucherForm, setVoucherForm] = useState({ instituteId: "", courseId: "any", name: "", quantity: "25", expiresAt: yearFromNow() });
  ...
  const createBatch = useMutation({
    mutationFn: async () => (await apiRequest("POST", "/api/admin/certification-voucher-batches", {
      instituteId: Number(voucherForm.instituteId),
    ...
    onSuccess: (result: { batch: VoucherBatch; codes: string[]; institute: { name: string } }) => {
      ...
      toast({ title: "Voucher batch issued", description: `${result.codes.length} one-time codes were generated for ${result.institute.name}; the secure CSV has been prepared.` });
  ...
  return <Tabs defaultValue="vouchers" className="space-y-5">
    ...
    <TabsContent value="vouchers" className="space-y-5">
      <Card className="border-violet-200"><CardHeader className="border-b border-violet-100 bg-violet-50/60"><CardTitle className="flex items-center gap-2 text-lg"><TicketCheck className="h-5 w-5 text-violet-700" />Issue institute voucher allocation</CardTitle><p className="text-sm leading-6 text-slate-600">Codes are revealed once and downloaded immediately. Every redemption is course-scoped, expiry-controlled, and audited.</p></CardHeader><CardContent className="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-3">
        <Field label="Verified institute"><select value={voucherForm.instituteId} onChange={(event) => setVoucherForm({ ...voucherForm, instituteId: event.target.value })} className={selectClass}><option value="">Select institute</option>{verifiedInstitutes.map((institute) => <option key={institute.id} value={institute.id}>{institute.name}</option>)}</select></Field>
    ...
    <TabsContent value="coupons" className="space-y-5">
      <Card><CardHeader className="border-b border-slate-100"><CardTitle className="flex items-center gap-2 text-lg"><Percent className="h-5 w-5 text-emerald-700" />Create governed coupon</CardTitle><p className="text-sm leading-6 text-slate-600">Coupons change a payable price. They never bypass a result, passing rule, or credential evidence policy.</p></CardHeader><CardContent className="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-3">
        ...
        <Field label="Certification scope"><select value={couponForm.courseId} onChange={(event) => setCouponForm({ ...couponForm, courseId: event.target.value })} className={selectClass}><option value="any">All certifications</option>{(coursesQuery.data || []).map((course) => <option key={course.id} value={course.id}>{course.title}</option>)}</select></Field>
