...
export function CertificationCard({
  ...
  const saleSavings = pricing.isCredentialOnSale
    ? Number(item.originalPrice) - Number(item.price)
    : 0;
  const savingsLabel = saleSavings > 0
    ? new Intl.NumberFormat("en-IN", {
        style: "currency",
        currency: "INR",
        maximumFractionDigits: Number.isInteger(saleSavings) ? 0 : 2,
      }).format(saleSavings)
    : null;
  ...
  return (
    <article
      ...
      className={`group relative flex h-full flex-col overflow-hidden rounded-[1.5rem] transition duration-300 hover:-translate-y-1 ${
        practice
          ? "border-2 border-slate-300 bg-slate-50 shadow-[0_14px_38px_-25px_rgba(15,23,42,0.45)] hover:border-slate-500 hover:shadow-slate-900/15"
    ...
    >
      {practice && <div aria-hidden className="absolute inset-x-6 top-0 z-20 border-t-4 border-dashed border-slate-400" />}
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        {metadata.length > 0 && (
          <div className="mt-5 grid grid-cols-2 gap-2 border-y border-slate-100 py-3 text-xs text-slate-600">
            {metadata.map(({ icon: Icon, label }) => <span key={label} className="flex items-center gap-1.5"><Icon className="h-3.5 w-3.5 text-slate-800" aria-hidden="true" />{label}</span>)}
          </div>
        )}

        <div className={`mt-auto rounded-2xl p-4 ${practice ? "border border-dashed border-slate-400 bg-white" : "border border-slate-300 bg-slate-50"}`}>
          ...
          {!practice && <p className="mt-2 text-xs font-bold text-slate-800">Browse as a guest · register or login only when starting</p>}
          {!practice && pricing.credentialPrice && (
            <div className="mt-3 flex flex-wrap items-baseline gap-x-2 border-t border-slate-300 pt-3">
              ...
              {savingsLabel && <span className="rounded-full bg-slate-200 px-2 py-0.5 text-[10px] font-black uppercase text-slate-900">Save {savingsLabel}</span>}
            </div>
          )}
        </div>

        <Button asChild className="mt-4 w-full rounded-full bg-slate-950 hover:bg-slate-800">
          <Link href={href} aria-label={`View exam details: ${item.title}`}>
            View exam details<ArrowUpRight className="ml-1.5 h-4 w-4" aria-hidden="true" />
          </Link>
        </Button>
      </div>
    </article>
