  ...
  return (
    ...
    >
      <button
        ref={triggerRef}
        type="button"
        className={`inline-flex min-h-11 items-center gap-1 rounded-xl px-3 text-sm font-bold transition-colors ${current || open ? "bg-slate-50 text-slate-800" : "text-slate-700 hover:bg-slate-50 hover:text-slate-950"}`}
        aria-expanded={open}
        ...
        >
          <div className="grid max-h-[min(38rem,calc(100vh-6rem))] overflow-y-auto overscroll-contain rounded-[1.5rem] border border-slate-200 bg-white text-left shadow-2xl shadow-slate-950/20 lg:grid-cols-[14rem_minmax(0,1.15fr)_minmax(18rem,.85fr)]">
            <section className="bg-slate-950 p-5 text-white" aria-labelledby="catalog-menu-heading">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-slate-400/20 text-slate-200">
...
function CatalogSection({
  ...
  return (
    <section className={`p-5 ${certification ? "border-r border-slate-200" : "bg-slate-50/45"}`} aria-labelledby={`${kind}-menu-heading`}>
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className={`flex items-center gap-2 text-xs font-black uppercase tracking-[0.14em] ${certification ? "text-slate-700" : "text-slate-800"}`}>
        ...
        <Link
          ...
          href={hub}
          className={`inline-flex shrink-0 items-center gap-1 rounded-full px-3 py-2 text-xs font-bold ${certification ? "bg-slate-100 text-slate-900 hover:bg-slate-200" : "bg-slate-100 text-slate-950 hover:bg-slate-200"}`}
        >
      ...
      {loading ? (
        ...
        <p role="status" className="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs leading-5 text-slate-900">This catalog is temporarily unavailable. Use “View all” to try again.</p>
      ) : groups.length === 0 ? (
          ...
          <nav aria-label={`${title} most populated categories`} className="mt-5 grid grid-cols-2 gap-2">
            {groups.slice(0, MENU_CATEGORY_LIMIT).map((group) => (
              <Link
                ...
                className={`group flex min-h-14 items-center justify-between gap-2 rounded-xl border bg-white px-3 py-2 transition ${certification ? "border-slate-100 hover:border-slate-300 hover:bg-slate-50" : "border-slate-100 hover:border-slate-300 hover:bg-slate-50"}`}
              >
                <span className="text-xs font-bold leading-4 text-slate-800">{group.name}</span>
                <span aria-label={`${group.items.length} assessments`} className={`rounded-full px-2 py-1 text-[10px] font-black ${certification ? "bg-slate-100 text-slate-800" : "bg-slate-100 text-slate-900"}`}>{group.items.length}</span>
              </Link>
            ))}
          </nav>
...
function MobileCatalogSection({ kind, total, groups, loading, error, onNavigate }: {
  ...
  const categoryPath = certification ? publicAssessmentCategoryPath : publicPracticeCategoryPath;
  return (
    <details className={`group overflow-hidden rounded-2xl border ${certification ? "border-slate-200 bg-slate-50" : "border-slate-200 bg-slate-50"}`}>
      <summary className="flex min-h-14 cursor-pointer list-none items-center justify-between gap-3 px-4 text-sm font-black text-slate-950">
        <span className="flex items-center gap-2">{certification ? <Award className="h-4 w-4 text-slate-700" /> : <BadgeIndianRupee className="h-4 w-4 text-slate-800" />}{certification ? "Certifications" : "Practice Pass"}<span className="text-xs font-semibold text-slate-500">{loading ? "…" : total ?? 0}</span></span>
      ...
      <div className="border-t border-slate-200 bg-white p-2">
        ...
        <Link href={hub} onClick={onNavigate} className={`flex min-h-11 items-center justify-between rounded-xl px-3 text-sm font-bold ${certification ? "text-slate-800 hover:bg-slate-50" : "text-slate-900 hover:bg-slate-50"}`}>Browse all <ArrowRight className="h-4 w-4" /></Link>
        {loading ? <div className="m-3 h-16 animate-pulse rounded-xl bg-slate-100" /> : error ? <p className="m-3 text-xs text-slate-800">Catalog temporarily unavailable.</p> : groups.length === 0 ? <p className="m-3 text-xs text-slate-500">No published assessments yet.</p> : groups.map((group) => (
