...
export function AdminQuestionBanksManagement() {
  ...
  return <div className="space-y-4">
    ...
    <Card className="border-slate-200 shadow-sm">
      ...
      <CardContent>
        {query.isLoading ? <div className="py-12 text-center text-sm text-slate-500">Loading question banks…</div> : query.isError ? <div className="rounded-xl border border-slate-200 bg-slate-50 p-5 text-sm text-slate-900">Question banks could not be loaded. <Button size="sm" variant="outline" className="ml-2" onClick={() => query.refetch()}>Retry</Button></div> : <>
          <div className="overflow-x-auto rounded-xl border border-slate-200"><Table><TableHeader><TableRow><TableHead className="w-10"><input aria-label="Select page" type="checkbox" checked={allSelected} onChange={() => setSelected(allSelected ? new Set() : new Set(items.map((item) => item.id)))} /></TableHead><TableHead>Bank</TableHead><TableHead>Classification</TableHead><TableHead>Publishable inventory</TableHead><TableHead>Usage</TableHead><TableHead>State</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader><TableBody>
            {items.map((bank) => <TableRow key={bank.id}>
              ...
              <TableCell><div className="font-semibold text-slate-950">{Number(bank.questionCount).toLocaleString()} publishable</div><div className="text-xs text-slate-500">{Number(bank.totalQuestionCount).toLocaleString()} current · {Number(bank.retiredQuestionCount).toLocaleString()} retired</div><div className="mt-1 flex flex-wrap gap-1 text-[11px]"><span className="rounded-full bg-slate-50 px-2 py-0.5 text-slate-800">E {Number(bank.easyCount).toLocaleString()}</span><span className="rounded-full bg-slate-50 px-2 py-0.5 text-slate-800">M {Number(bank.mediumCount).toLocaleString()}</span><span className="rounded-full bg-slate-50 px-2 py-0.5 text-slate-800">H {Number(bank.hardCount).toLocaleString()}</span></div></TableCell>
