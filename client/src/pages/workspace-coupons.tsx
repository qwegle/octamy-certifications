...
export default function WorkspaceCoupons({ role }: { role: WorkspaceRole }) {
  ...
  return <DashboardLayout role={role} title="Coupons" description="Run controlled promotions for products owned by this workspace." breadcrumbs={[{ label: role === "creator" ? "Creator" : "Institute", href: `/${role}/dashboard` }, { label: "Coupons" }]}>
    ...
    <div className="grid gap-6 xl:grid-cols-[420px_minmax(0,1fr)]">
      <Card className="h-fit overflow-hidden border-emerald-200 shadow-sm">
        <CardHeader className="border-b border-emerald-100 bg-emerald-50/70"><CardTitle className="flex items-center gap-2 text-lg"><Percent className="h-5 w-5 text-emerald-700" />Create coupon</CardTitle><p className="text-sm leading-6 text-slate-600">Coupons affect price only. Access and certificate rules still apply.</p></CardHeader>
