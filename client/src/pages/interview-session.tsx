...
export default function InterviewSession() {
  ...
  const interviewId = paramsInterview?.id
    ? parseInt(paramsInterview.id)
    : paramsInterviews?.id
    ? parseInt(paramsInterviews.id)
    : paramsResults?.id
    ? parseInt(paramsResults.id)
    : null;
  ...
  const startRecording = async () => {
    try {
      ...
      toast({
        ...
        description:
          "Could not start video recording. Please check your camera permissions.",
        variant: "destructive",
      });
    }
  };
  ...
  const startScreenRecording = async () => {
    try {
      ...
      screenRecordedChunksRef.current = []; // Reset chunks
      ...
      toast({
        ...
        description:
          "Could not start screen recording. Please check your permissions.",
        variant: "destructive",
      });
    }
  };
  ...
  const stopAllRecordings = async () => {
    try {
      ...
      // Return the recorded chunks if needed
      return {
        cameraChunks: recordedChunksRef.current,
        screenChunks: screenRecordedChunksRef.current,
      };
    } catch (error) {
      console.error("Error in stopAllRecordings:", error);
      setIsRecording(false);
      setIsScreenRecording(false);
      ...
      toast({
        title: "Recording Error",
        description:
          error instanceof Error ? error.message : "Failed to stop recordings",
        variant: "destructive",
      });
  ...
  const uploadRecording = async (chunks1: Blob[], chunks2: Blob[]) => {
    try {
      ...
      formData.append("screen", blob1, "screen-recording.webm");
      formData.append("video", blob2, "camera-recording.webm");
  ...
  const submitInterviewMutation = useMutation({
    mutationFn: async () => {
      // First stop all recordings and get the chunks
      const { cameraChunks, screenChunks } = await stopAllRecordings();

      // Upload recordings sequentially to avoid overloading
      const result =
        screenChunks.length > 0 && cameraChunks.length > 0
          ? await uploadRecording(cameraChunks, screenChunks)
          : null;
      ...
      // Submit interview data with the URLs
      const response = await fetch(`/api/interviews/${interviewId}/submit`, {
        ...
        body: JSON.stringify({
          ...
          screenRecordingUrl: result?.screenUrl,
        }),
      });
    ...
    onError: (error: Error) => {
      toast({
        ...
        description:
          error.message || "Failed to submit interview. Please try again.",
        variant: "destructive",
      });
    },
  });
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <span
                    className={`inline-flex px-3 py-1 rounded-full text-sm font-medium ${getGradeColor(
                      interview.grade || "F"
                    )}`}
                  >
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <p className="font-semibold">{interview.aiSummary || 0}</p>
                </div>
                <div className="bg-gray-50 p-4 rounded-lg">
                ...
                {interview.screenRecordingUrl && (
                  <>
                    ...
                    <video
                      controls
                      className="w-full max-w-2xl rounded-lg shadow-lg"
                      src={interview.screenRecordingUrl}
                    />
                  </>
                )}
              </CardContent>
      ...
      <div className="min-h-screen bg-white">
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
          <Card>
            ...
            <CardContent className="space-y-4">
              <p>
                Get ready for your {interview.technology} interview. Make sure
  ...
  const progress = interview.questions
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-orange-700">
                            This is a hands-on coding question. Your screen will
                            be recorded during this question to evaluate your
