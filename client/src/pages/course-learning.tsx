...
export default function CourseLearning() {
  ...
  if (courseLoading || curriculumLoading) return <div className="min-h-screen bg-[#f5f5f5]"><Header /><div className="mx-auto max-w-7xl px-6 py-20"><div className="h-12 w-2/3 animate-pulse rounded-xl bg-slate-200" /><div className="mt-8 h-96 animate-pulse rounded-3xl bg-slate-200" /></div></div>;
  if (!course) return <div className="min-h-screen bg-[#f5f5f5]"><Header /><main className="mx-auto max-w-3xl px-6 py-24 text-center"><h1 className="text-4xl font-black">Course not found</h1><Link href="/courses"><Button className="mt-6">Browse catalog</Button></Link></main><Footer /></div>;
      ...
      <main id="main-content">
        <section className="border-b border-white/10 bg-slate-950 text-white">
          <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
            <div className="flex flex-wrap gap-2"><Badge className="bg-slate-500/20 text-slate-100 hover:bg-slate-500/20">{access?.productType === "bundle" ? "Course + assessment" : "Video course"}</Badge><Badge variant="outline" className="border-white/20 text-slate-200">{course.level}</Badge></div>
            ...
            <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl shadow-slate-900/5">
              <div className={selected?.kind === "pdf" && !selected.locked ? "min-h-[70vh] bg-slate-950" : "aspect-video bg-slate-950"}>
                {!selected ? <div className="grid h-full place-items-center text-slate-400">No lessons have been published yet.</div>
                  : selected.locked ? <div className="grid h-full place-items-center p-8 text-center"><div><LockKeyhole className="mx-auto h-10 w-10 text-slate-300" /><h2 className="mt-5 text-2xl font-black text-white">This lesson is protected</h2><p className="mt-2 text-slate-300">Activate course access to open non-preview content.</p><Button onClick={unlock} className="mt-6 bg-white text-slate-950 hover:bg-slate-100">{access?.requiresPurchase ? `Buy access · ₹${Number(access.contentPrice).toLocaleString("en-IN")}` : "Enrol free"}</Button></div></div>
                  ...
                  : ["video", "pdf"].includes(selected.kind) && streamError ? <div className="grid h-full min-h-80 place-items-center p-8 text-center"><div><LockKeyhole className="mx-auto h-10 w-10 text-slate-300" /><h2 className="mt-4 text-xl font-black text-white">Lesson media could not be opened</h2><p className="mt-2 max-w-lg text-sm text-slate-300">{streamError}</p><Button type="button" onClick={() => setStreamRetry((value) => value + 1)} className="mt-5 bg-white text-slate-950 hover:bg-slate-100">Try again</Button></div></div>
                  ...
                  : selected.contentUrl ? <div className="grid h-full place-items-center p-8 text-center"><FileText className="mx-auto h-10 w-10 text-slate-300" /><p className="mt-4 text-white">External lesson resource</p><a href={selected.contentUrl} target="_blank" rel="noreferrer"><Button className="mt-5 bg-white text-slate-950">Open resource <ArrowRight className="ml-2 h-4 w-4" /></Button></a></div>
                  : <div className="grid h-full place-items-center text-slate-400">Content has not been attached to this lesson.</div>}
              </div>
              {selected && <div className="p-5 sm:p-6"><div className="flex items-center gap-2"><Badge variant="outline" className="capitalize">{selected.kind}</Badge>{selected.isPreview && <Badge className="bg-slate-100 text-slate-800">Free preview</Badge>}</div><h2 className="mt-3 text-2xl font-black">{selected.title}</h2>{["video", "pdf"].includes(selected.kind) && !selected.locked && <p className="mt-3 max-w-3xl text-xs leading-5 text-slate-500">Access is account-gated and browser download controls are reduced. As with every web learning platform, these controls discourage casual copying but cannot guarantee prevention of screen capture or device-level recording.</p>}</div>}
            </div>

            {access?.productType === "bundle" && <Card className="mt-6 border-slate-200 bg-slate-50"><CardContent className="flex flex-col gap-4 p-6 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-black text-slate-950">Ready to validate the skill?</h2><p className="mt-1 text-sm text-slate-900/70">The assessment and optional credential remain separate from course-content access.</p></div><Link href={`/get-certified/${course.slug}`}><Button className="bg-slate-700 text-white">Take certification exam</Button></Link></CardContent></Card>}
          </div>

          <aside className="h-fit rounded-3xl border border-slate-200 bg-white p-4 shadow-sm lg:sticky lg:top-24">
            <div className="flex items-center justify-between px-2 py-2"><h2 className="font-black">Course content</h2>{access?.hasAccess && <span className="flex items-center gap-1 text-xs font-bold text-slate-700"><CheckCircle2 className="h-4 w-4" />Unlocked</span>}</div>
            <div className="mt-2 max-h-[65vh] space-y-4 overflow-y-auto pr-1">
              {sections.map((section) => <div key={section.id}><p className="px-2 text-xs font-black uppercase tracking-wider text-slate-400">{section.title}</p><div className="mt-1 space-y-1">{section.lessons.map((lesson) => <button key={lesson.id} onClick={() => setSelectedId(lesson.id)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm transition ${selected?.id === lesson.id ? "bg-slate-950 text-white" : "hover:bg-slate-100"}`}>{lesson.locked ? <LockKeyhole className="h-4 w-4 shrink-0" /> : lesson.kind === "video" ? <PlayCircle className="h-4 w-4 shrink-0" /> : <FileText className="h-4 w-4 shrink-0" />}<span className="min-w-0 flex-1 truncate">{lesson.title}</span>{lesson.isPreview && <span className="text-[10px] font-bold uppercase text-slate-600">Preview</span>}</button>)}</div></div>)}
            </div>
            {!access?.hasAccess && access?.requiresPurchase && (
              <div className="mt-5 rounded-2xl bg-slate-50 p-3">
                <label htmlFor="course-coupon" className="flex items-center gap-2 text-xs font-black uppercase tracking-wider text-slate-500"><TicketPercent className="h-4 w-4 text-slate-600" />Coupon</label>
                ...
                {couponQuote && <p className="mt-2 text-xs font-bold text-slate-700">Applied {couponQuote.codeHint} · save ₹{Number(couponQuote.discountAmount).toLocaleString("en-IN")}</p>}
              </div>
            )}
            {!access?.hasAccess && access?.productType !== "assessment" && <Button onClick={unlock} disabled={freeEnrol.isPending || paidCheckout.isPending} className="mt-5 w-full bg-slate-700 text-white hover:bg-slate-800">{access?.requiresPurchase ? `Buy course · ₹${Number(couponQuote?.finalAmount ?? access.contentPrice).toLocaleString("en-IN")}` : "Enrol free"}</Button>}
          </aside>
        </section>
      </main>
      <Footer />
