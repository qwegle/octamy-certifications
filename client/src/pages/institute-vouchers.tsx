      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
        <Card className="h-fit border-slate-200 shadow-sm">
          <CardHeader className="border-b border-slate-100 bg-slate-50/70">
            <CardTitle className="flex items-center gap-2 text-lg"><Mail className="h-5 w-5 text-slate-700" />Assign to a learner</CardTitle>
          ...
          <CardContent className="space-y-4 p-5">
            ...
            <p className="flex gap-2 border-t border-slate-100 pt-4 text-xs leading-5 text-slate-500"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-slate-600" />Codes are not stored in readable form. Keep the original allocation file in your approved secure workspace.</p>
          </CardContent>
        </Card>
...
function SummaryCard({ icon: Icon, label, value, loading }: { icon: typeof TicketCheck; label: string; value: number; loading: boolean }) {
  return <Card><CardContent className="flex items-center gap-4 p-5"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-slate-50 text-slate-700"><Icon className="h-5 w-5" /></span><div><p className="text-sm text-slate-500">{label}</p><p className="mt-0.5 text-2xl font-black text-slate-950">{loading ? "—" : value}</p></div></CardContent></Card>;
}
...
function VoucherBatchCard({ batch }: { batch: VoucherBatch }) {
  ...
  return <Card className="overflow-hidden"><CardContent className="p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.12em] text-slate-700">{batch.courseTitle || "Any eligible in-house certification"}</p><h3 className="mt-2 font-black text-slate-950">{batch.name}</h3></div><Badge variant="outline" className={active ? "border-slate-200 bg-slate-50 text-slate-800" : "border-slate-200 bg-slate-50 text-slate-600"}>{active ? "Active" : batch.status}</Badge></div><div className="mt-5 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-slate-600" style={{ width: `${usedPercent}%` }} /></div><div className="mt-3 grid grid-cols-3 gap-2 text-center"><div><p className="text-lg font-black">{batch.available}</p><p className="text-[11px] text-slate-500">Available</p></div><div><p className="text-lg font-black">{batch.assigned}</p><p className="text-[11px] text-slate-500">Assigned</p></div><div><p className="text-lg font-black">{batch.redeemed}</p><p className="text-[11px] text-slate-500">Redeemed</p></div></div><p className="mt-4 flex items-center gap-1.5 border-t border-slate-100 pt-4 text-xs text-slate-500"><CalendarClock className="h-3.5 w-3.5" />Expires {new Date(batch.expiresAt).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</p></CardContent></Card>;
}
