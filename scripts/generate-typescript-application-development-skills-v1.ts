...
const DRAFTS: Draft[] = [
  ...
  q("control-flow-narrowing", "Why does checking `if (input)` not prove that `input: string | null` is a non-empty business identifier?", "Truthiness removes null but also excludes the valid empty string without documenting business validation", ["Truthiness cannot narrow union types", "Strings are always falsy in JavaScript", "The check converts `input` to a number"], "Truthiness narrowing follows JavaScript coercion; an explicit null check and separate domain validation are clearer when empty strings have meaning.", REFERENCES.narrowing, "Recognize limits of truthiness narrowing", "medium"),
