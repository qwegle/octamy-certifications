...
const SYLLABUS = "OCT-ADMS-2026.1 (HTTP, OpenAPI 3.2, OWASP API 2023, 12-Factor, Kubernetes; 2026-07-28)";
...
const REFERENCES = {
  ...
  twelveCodebase: "https://12factor.net/codebase",
  twelveConfig: "https://12factor.net/config",
  twelveDependencies: "https://12factor.net/dependencies",
  twelveProcesses: "https://12factor.net/processes",
  twelveLogs: "https://12factor.net/logs",
...
const DRAFTS: Draft[] = [
  ...
  q("http-semantics-resource-design", "A client needs to retry an update after a lost response, and the operation can be expressed as replacing the target resource with the same representation. Which HTTP method best matches that retry property?", "PUT", ["POST", "CONNECT", "PATCH without defined idempotent semantics"], "PUT is defined as idempotent, so repeating the same replacement request has the same intended effect on the target resource.", REFERENCES.httpSemantics, "Choose an idempotent replacement method", "hard"),
  ...
  q("service-boundaries-data-ownership", "What does the Twelve-Factor share-nothing process model imply for user session state in a horizontally scaled API?", "Store durable session state in a backing service rather than in one process's local memory", ["Store sessions only in the first process that receives the user", "Require all traffic for a user to stay on one container forever", "Commit session files into the application repository"], "Twelve-Factor processes are stateless and share-nothing; persistent state belongs in backing services so any process can serve work.", REFERENCES.twelveProcesses, "Apply stateless process design", "easy"),
  q("service-boundaries-data-ownership", "Which storage choice violates the Twelve-Factor process model for persistent customer profile data?", "Keeping the only copy in a web process's memory", ["Reading it from a durable backing database", "Fetching it from an attached backing service", "Passing an identifier through the request"], "Runtime processes can start, stop, and be replaced. Persistent data must not live only inside one disposable process.", REFERENCES.twelveProcesses, "Keep persistent data out of processes", "easy"),
  ...
  q("service-boundaries-data-ownership", "Why should a scaled API not treat an in-memory map inside one process as the system of record?", "Twelve-Factor processes are disposable and share-nothing, so another instance or restart would not have that state", ["HTTP forbids maps in application code", "OpenAPI cannot document data stored in memory", "Kubernetes Services only route to stateful processes"], "A process can be stopped or replaced and other processes do not share its memory. Durable state belongs in backing services.", REFERENCES.twelveProcesses, "Avoid process-local source of truth", "medium"),
  q("service-boundaries-data-ownership", "A service receives SIGTERM during deployment. What should it do under the Twelve-Factor disposability principle?", "Stop accepting new work, finish or safely release in-flight work, and exit promptly", ["Ignore the signal until the node is restarted", "Delete its backing database before exit", "Write deploy configuration into source code"], "Disposability emphasizes graceful shutdown and fast process termination so deploys and scaling do not corrupt work.", REFERENCES.twelveDisposability, "Handle termination gracefully", "medium"),
  ...
  q("service-boundaries-data-ownership", "Which logging behavior matches Twelve-Factor service design?", "Write logs as event streams to stdout or stderr and let the execution environment route them", ["Open and rotate application log files as the only log sink", "Store request logs only in a process-local array", "Disable logs so processes remain stateless"], "The Logs factor treats logs as event streams; the app writes events and the environment handles collection and routing.", REFERENCES.twelveLogs, "Emit logs as event streams", "medium"),
  q("service-boundaries-data-ownership", "A team maintains separate repositories `orders-dev`, `orders-staging`, and `orders-prod` for the same service. Which Twelve-Factor rule does this violate?", "One codebase should be tracked in revision control and used for many deploys", ["Every deploy must have a different codebase", "A codebase may not have more than one API endpoint", "Revision control should contain runtime secrets"], "The Codebase factor requires one codebase per app with many deploys, not environment-specific code forks.", REFERENCES.twelveCodebase, "Preserve one codebase across deploys", "hard"),
  q("service-boundaries-data-ownership", "A service works only on one server because it silently depends on a globally installed image library that is not declared by the app. What is the Twelve-Factor defect?", "Dependencies are not explicitly declared and isolated, so deploys are not reproducible", ["The service has too many HTTP status codes", "The API contract has too many schemas", "The process is too stateless"], "The Dependencies factor requires explicit declaration and isolation so environments can be reproduced without hidden system packages.", REFERENCES.twelveDependencies, "Declare service dependencies", "hard"),

  q("resilience-observability-failure", "What operational behavior does the Twelve-Factor disposability principle expect from service processes?", "Fast startup and graceful shutdown", ["Permanent local disk state inside each process", "Manual startup only during business hours", "Ignoring termination signals during deploys"], "Disposability says processes should start quickly and shut down gracefully so releases, scaling, and recovery are reliable.", REFERENCES.twelveDisposability, "Explain disposability", "easy"),
  ...
  q("resilience-observability-failure", "Why is automatic retry of an unchanged PUT generally safer than automatic retry of a side-effecting POST?", "PUT is defined as idempotent, while POST does not have that general idempotent guarantee", ["POST is always a read-only method", "PUT bypasses server authorization", "HTTP requires caches to retry PUT forever"], "RFC 9110 defines PUT as idempotent but not POST, so retry policy must account for the method's intended effect.", REFERENCES.httpSemantics, "Compare retry risk by method semantics", "medium"),
  ...
  q("resilience-observability-failure", "Which logging approach best matches the Twelve-Factor Logs principle for services running in containers?", "Write each log event to stdout or stderr and let the platform collect and route the stream", ["Write only to a rotating file inside the container filesystem", "Store logs in process memory until shutdown", "Require each request handler to open the production log database directly"], "Twelve-Factor apps do not manage log files; they emit event streams that the execution environment captures.", REFERENCES.twelveLogs, "Apply container logging model", "medium"),
  ...
  q("resilience-observability-failure", "A client times out after sending a non-idempotent POST. What can it safely infer from HTTP semantics alone?", "It cannot know whether the server applied the requested processing, so blind retry can duplicate side effects", ["The server definitely did not receive the request", "The method becomes safe after the timeout", "The response must be reused from a shared cache"], "POST is not generally idempotent, and a lost response does not prove the origin server did no work.", REFERENCES.httpSemantics, "Reason about ambiguous POST outcomes", "hard"),
  ...
  q("configuration-secrets-delivery", "A team changes `PAYMENT_GATEWAY=mock` in production to bypass an outage and forgets to restore it. What control best prevents this class of configuration accident?", "Use governed runtime config with review, audit trail, and environment-specific validation for high-risk values", ["Hide the setting in source code so nobody can change it", "Let each pod choose its own gateway independently", "Store the value only in frontend localStorage"], "Runtime config is powerful and must be controlled. Sensitive operational switches need validation and auditability, not hard-coded bypasses.", REFERENCES.twelveConfig, "Govern high-risk config", "hard"),
