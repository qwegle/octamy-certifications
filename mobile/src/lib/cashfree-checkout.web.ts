...
export async function openCashfreeCheckout(input: CashfreeCheckoutInput): Promise<CashfreeCheckoutResult> {
  if (typeof window === 'undefined') {
    throw new ApiError({ status: 0, code: 'CHECKOUT_UNAVAILABLE', message: 'Cashfree checkout is available only in the browser.' });
  }
  window.location.assign(checkoutUrl(input));
  return { type: 'opened' };
}
